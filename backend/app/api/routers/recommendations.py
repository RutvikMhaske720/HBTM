...
from app.api.schemas import FeedbackPayload, RecommendationOut, RunTriggerResponse, ScoreBreakdown
...
def refresh_recommendations(body: dict, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    """
...
def submit_feedback(content_id: str, payload: FeedbackPayload, db: Session = Depends(get_db)):
